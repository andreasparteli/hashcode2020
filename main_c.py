...
def read(filepath):
    ...
    for id in range(0, n_libs):
        ...
        libs.append({'id': id, 'signup': lsignup, 'ships': lships, 'books': set(lbooks)})
...
def score(books, libs_to_scan):
    ...
    for libts in libs_to_scan:
        this_books = set(libts['books'])
        duplicates = this_books & all_books
        if duplicates:
            print(f"Duplicate books found!! {duplicates}")
        all_books |= this_books
    score = 0
    for b in all_books:
        score += books[b]
    print(f"Score for {len(all_books)} scanned books is {score}")
    return score
...
for d in range(n_days):
    if d == next_signup and d < n_days - 1:
        ...
        if books_to_pick > 0:
            to_pick = list(pick['books'] - all_picked_books)
            ...
            all_picked_books |= set(pickedbooks)
...
write(libtsinorder, outfilepath)